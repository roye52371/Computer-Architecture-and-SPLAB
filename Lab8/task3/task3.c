...
void the_Relocation_Tables(state* s);
...
struct fun_desc menu[]= {{"Toggle Debug Mode",ToggleDebugMode},{"Examine ELF File",ExamineELFFile},{"Print Section Names",the_PrintSectionNames},{"Print Symbols",the_Print_Symbols},{"Relocation Tables",the_Relocation_Tables},{"Quit",quit},{NULL,NULL}};
...
void ExamineELFFile(state* s){
    ...
    if(header->e_ident[EI_MAG1]!=ELFMAG1 || header->e_ident[EI_MAG2]!=ELFMAG2 || header->e_ident[EI_MAG3]!=ELFMAG3){
        fprintf(stderr,"the magic number/s is not consistent with an ELF file\n");
...
void the_Relocation_Tables(state* s) {
    ...
    char *beginonetable = NULL;//beginsymbolname
    Elf32_Sym *beginsymboltalb = NULL;//beginsymboltalb
    ...
    Elf32_Shdr *sechead =  map_start + header->e_shoff;//sectionhead
    char *beginthe_table_sec = map_start + sechead[header->e_shstrndx].sh_offset;//beginsectionname


    int k=0;
    int foundedDynsym=0;
    while (k < header->e_shnum && foundedDynsym==0) {
        if (sechead[k].sh_type == SHT_DYNSYM) {//if dynsym than founded the symbol we need
            beginsymboltalb = map_start + sechead[k].sh_offset;
            beginonetable =  map_start + sechead[sechead[k].sh_link].sh_offset;
            foundedDynsym=1;
        }
        k=k+1;
    }
    ...
    for (int j = 0; j < header->e_shnum; j=j+1) {
        if (sechead[j].sh_type == SHT_REL) {
            int size =sechead[j].sh_size ;
            printf("Relocation section\t %s\tat offset 0x%03X\tcontains %d\t enteries\n\n",
                   beginthe_table_sec + sechead[j].sh_name,
                   sechead[j].sh_offset, size / sechead[j].sh_entsize);
            Elf32_Rel *beginReltable = map_start + sechead[j].sh_offset;
            ...
            int t=0;
            while(t < size/sechead[j].sh_entsize) {
                int our_symbol = ELF32_R_SYM(beginReltable[t].r_info);
                int our_type = ELF32_R_TYPE(beginReltable[t].r_info);
                printf("%08x\t %08x\t %d\t\t %08x\t %s\n", beginReltable[t].r_offset, beginReltable[t].r_info, our_type,
                       beginsymboltalb[our_symbol].st_value, beginonetable + beginsymboltalb[our_symbol].st_name);
                t=t+1;
            }
