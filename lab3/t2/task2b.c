           ...
           // char cleanbuffer2='0';
            if(decision==1){
               ...
               // list_free(link);
                ...
                char begByte[10];// at most 10000
                char sizeSig[10];
                int beginByte;
                int sigSize;
                printf("please enter starting byte location of the infected virus in the suspected file: ");
                fgets(begByte,10,stdin);
                //printf("\n");
                printf("please enter signature size of the infected virus in the suspected file: ");
                fgets(sizeSig,10,stdin);
                //printf("\n");
                beginByte=atoi(begByte);
                sigSize=atoi(sizeSig);
                fseek(stdin,0,SEEK_SET);
                kill_virus(argv[1],beginByte,sigSize);
...
void kill_virus(char *fileName, int signitureOffset, int signitureSize){
    FILE *fs;
    fs = fopen(fileName, "r+");//r+ read write with out delete what was, b, means byte by byte for binary files
    if (fs == NULL) {
        fprintf(stderr, "file does not exist\n");

    }
    else{
        fseek(fs,signitureOffset,SEEK_SET);// set the begin to be the start if a file so we start overwrite bytes from there
        char NOPInstruction[signitureSize];
        for(int i=0; i< signitureSize; i=i+1){
         NOPInstruction[i]=NOP;
    
    
        }
        
       // size_t oneByte= sizeof(char);// char is 1 byte
        
         fwrite(NOPInstruction,signitureSize,1,fs);
